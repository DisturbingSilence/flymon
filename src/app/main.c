...
void oled_task(void* ctx)
{
    ...
    char buf[50];
    sprintf(buf,"COUNTER:%d",gctx->counter);
...
static void buttons_callback(void* ctx,button_t btn)
{
    global_context_t* gctx = (global_context_t*)ctx;
    if(btn == BUTTON_LEFT) gctx->counter--;
    else if(btn == BUTTON_RIGHT) gctx->counter++;
    else if(btn == BUTTON_SELECT) gctx->counter = 0;
}
